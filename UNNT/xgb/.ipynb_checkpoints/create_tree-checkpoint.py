...
class Tree:
    ...
    def train(self):
        ...
        self.runtime = round(self.end_time - self.start_time, 2)
        print("Training Time: ", self.runtime, " seconds")

    def evaluate(self):
        self.predictions = self.xgb_model.predict(self.x_test)
        self.rmse = round(np.sqrt(MSE(self.y_test, self.predictions)), 4)
        print("RMSE : % f" %(self.rmse))
        self.r2_error = round(r2_score(self.y_test, self.predictions), 4)
